<template>
  <div id="table" class="app-container calendar-list-container">
    ...
    <div class="filter-container" style="margin: 10px 10px 10px 0;">
      ...
      />
      <el-button v-permission="'/picture/add'" class="filter-item" type="primary" icon="el-icon-edit" @click="getPictureSpiderList">开始爬取</el-button>
      <el-button v-permission="'/picture/delete'" class="filter-item" type="success" icon="el-icon-delete" @click="handleUploadPicture">上传图片管理</el-button>
    ...
    <el-row>
      ...
      >
        <el-card
          ...
          shadow="always"
        >
          <input :id="url" :checked="pictureUrls.indexOf(url)>=0" style="position: absolute;z-index: 100;" type="checkbox" @click="checked(url)">
          <el-image
            :src="url"
    ...
    <!-- 添加或修改对话框 -->
    <el-dialog :visible.sync="dialogFormVisible" title="上传至图片管理">
      <el-form ref="form" :model="form" :rules="rules">

        <el-form-item :label-width="formLabelWidth" label="上传的图片分类" prop="pictureSortUid">
          <el-select
...
export default {
  ...
  methods: {
    getPictureSpiderList: function() {
      ...
      pictureSpider(params).then(response => {
        if (response.code == this.$ECode.SUCCESS) {
          const spiderPictureList = response.data
    ...
    handleDeleteBatch: function() {
      ...
      const pictureUrls = this.pictureUrls
      const spiderPictureList = this.spiderPictureList
    ...
    checked: function(url) {
      const idIndex = this.pictureUrls.indexOf(url)
      if (idIndex >= 0) {
        // 选过了
            ...
            if (response.code == that.$ECode.SUCCESS) {
              ...
              addPicture(pictureUploadList).then(res => {
                if (res.code == that.$ECode.SUCCESS) {
                  ...
                  // 删除上传完的图片
                  const pictureUrls = that.pictureUrls
                  const spiderPictureList = that.spiderPictureList
                  for (let a = 0; a < pictureUrls.length; a++) {
                    spiderPictureList.remove(pictureUrls[a])
                  }
